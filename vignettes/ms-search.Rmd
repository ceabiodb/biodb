...
```
The returned structure is a list containing three fields, each ordered from greatest matching score to lowest:

 * `measure`: The matching score.
 * `matchedpeaks`: Each column corresponds to a peak of the searched spectrum (from first to last peak). A `-1` means that the peak has not been matched. An integer N greater than `0` means that the peak has been matched with the Nth peak of the database spectrum.
 * `id`: Matched database spectra identifiers.
