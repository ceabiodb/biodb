...
```{r, code=readLines('includes/vignettes_ref.R'), echo=FALSE}
```
...
In R, connectors to databases, when they exist, are presented in separate
packages, each proposing its own API.
When such a package does not offer the services the scientific programmer
...
<!-- TODO ref revue des packages existants OU liste des packages existants.
note spéciale sur webchem
-->
...
See vignette
```{r, echo=FALSE, results='asis'}
make_vignette_ref('ms_annotation')
```
...
See vignette
```{r, echo=FALSE, results='asis'}
make_vignette_ref('in_house_lcms_db')
```
...
See vignette
```{r, echo=FALSE, results='asis'}
make_vignette_ref('in_house_msms_db')
```
...
<!-- Insert links to database website in "Description" column. -->
...
<!-- TODO use knitr::kable(). Remove name column, and insert a link to vignette HTML file. -->
