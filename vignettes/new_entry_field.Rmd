...
For the example, we will use a connector to the ChEBI database:
```{r}
...
```{r}
defFile <- system.file("extdata", "new_entry_field.yml", package = "biodb")
```
Here is its content:
```{r}
...
conn <- mybiodb$getFactory()$getConn('chebi')
```
This allows the new connector to have new parsing expression registered, as we check it here:
```{r}
conn$getPropertyValue('parsing.expr')
```
