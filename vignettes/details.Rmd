...
If the key's value is boolean, you can use the following method instead:
```{r}
if (mybiodb$getConfig()$isEnabled('offline')) 'Biodb is running offline.' else 'Biodb is running online.'
```

To get a complete list of all keys with their description, call:
```{r}
...
mybiodb$getConfig()$get('cache.directory') # See modifications
```

If the field is boolean, you can use the following methods instead:
```{r}
mybiodb$getConfig()$enable('offline')    # set to TRUE
mybiodb$getConfig()$disable('offline')   # set to FALSE
```
