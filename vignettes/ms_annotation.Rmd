...
We will use the ChEBI database for this example:
...
inhouse <- mybiodb$getFactory()$createConn('mass.csv.file', url=lcmsdb)
```
...
inhouse$addField('ms.level', 1)
inhouse$addField('chrom.rt.unit', 's')
```
...
inhouse$searchMsPeaks(mzdf, mz.tol=1e-3, fields=c('accession', 'name', 'formula', 'chebi.id'), prefix='mydb.')
```
...
inhouse$searchMsPeaks(mzdf, mz.tol=1e-3, fields=fields, prefix='mydb.', chrom.col.ids=chromColIds, rt.unit='s', rt.tol=10, match.rt=TRUE)
```
