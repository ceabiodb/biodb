...
To obtain an entry from a database you have to call the `getEntry()` method from the `BiodbFactory` class. The following example access the ChemSpider database in order to retrieve the entry number 2:
```{r}
entry <- mybiodb$getFactory()$getEntry('chemspider', id = '2')
```
