...
![marcie](Marcie_from_Peanuts.png)
...
You can also set your own instance of `BiodbLogger`, using the `observers` argument:
```{r}
mybiodb <- biodb::Biodb$new(logger = FALSE, observers = biodb::BiodbLogger$new(verbose.level = as.integer(2)))
```
This will allow you to adjust the level of output you desire.

You can also redirect the output of the logger to a file:
```{r}
