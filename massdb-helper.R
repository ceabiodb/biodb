simplifySpectrum <- function(spec){
	if(nrow(spec)==0) stop("Empty spectrum.")
	if(ncol(spec)!=2){
		spec[,BIODB.PEAK.MZ]
		mint <- BIODB.GROUP.INTENSITY %in% colnames(libspec)
		pint <- which(mint[1])
		if(length(pint)==0)  stop("No intensity column founds, if there is more than 2 column, columns should be named",paste0(BIODB.GROUP.INTENSITY,collapse=", "))
...
calcDistance <- function(spec1 ,spec2, npmin=2, fun=MSMS.DIST,params=list()){
	fun <- match.arg(fun)
	...
	if(sum(res$matched!=-1)<npmin) return(list(matched=res$matched,similarity=0))
	list(matched=res$matched,similarity=res$measure)
}



###The returned sim list is not ordered 
compareSpectra <- function(spec,libspec,npmin=2, fun=MSMS.DIST,params=list(),decreasing=TRUE){
	fun <- match.arg(fun)
	if(length(libspec)==0){
		return(NULL)
	}
